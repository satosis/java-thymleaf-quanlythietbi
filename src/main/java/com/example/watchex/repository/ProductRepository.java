...
public interface ProductRepository extends JpaRepository<Devices, Integer> {
    ...
    @Query("SELECT p FROM Devices p " +
            "order by p.pro_pay desc")
    List<Devices> getProductsAccessoriess(Pageable pageable);

    @Query("SELECT p FROM Devices p " +
            "WHERE p.pro_active = 1 " +
            "order by p.pro_pay desc")
    List<Devices> getProductsGlass(Pageable pageable);

    @Query("SELECT p FROM Devices p " +
            "WHERE p.pro_active = 1 " +
            "order by p.pro_pay desc")
    List<Devices> getProductsWatch(Pageable pageable);

    @Query("SELECT p FROM Devices p " +
            "WHERE p.pro_active = 1 and p.category.id = :cate " +
            "order by p.pro_pay desc")
    List<Devices> getProductsByCategory(Integer cate, Pageable pageable);

    @Query("select p from Devices p where p.pro_slug = :slug")
    ProductDetailDto findBySlug(String slug);

    @Query("select p from Devices p where p.category.c_slug = :slug")
    Page<Devices> findBySlugCategory(String slug, Pageable pageable);

    @Query("select p from Devices p where p.pro_active = 1")
