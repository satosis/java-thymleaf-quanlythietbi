    ...
    public String changeStatus(@PathVariable("action") String action, @PathVariable("id") Integer id) throws ClassNotFoundException {
        ...
        if (Objects.equals(action, "confirm")) {
            ...
            for (Order order : orders) {
                Devices devices = productService.show(order.getOd_devices_id().getId());
                devices.setPro_pay(devices.getPro_pay() + 1);
                devices.setPro_amount(devices.getPro_amount() - order.getOd_qty());
            }
        }
        if (transaction != null) {
            switch (action) {
