...
public class TransactionController {
    ...
    public String store(@Valid @ModelAttribute("StoreTransactionDto") StoreTransactionDto storeTransactionDto, Model model) {
        ...
        for (int i = 0; i < Cart.cart.size(); i++) {
            ...
            order.setOd_devices_id(Cart.cart.get(i).devices);
            order.setOd_price(Cart.cart.get(i).devices.getPro_price());
            order.setOd_sale(Cart.cart.get(i).devices.getPro_sale());
