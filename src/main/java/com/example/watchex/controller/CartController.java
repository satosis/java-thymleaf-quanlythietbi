...
public class CartController {
    ...
    public String index(Model model) {
        ...
        for (int i = 0 ;i < Cart.cart.size(); i++) {
            total += Integer.parseInt(String.valueOf((Cart.cart.get(i).devices.getPro_price() - Cart.cart.get(i).devices.getPro_price() * (Cart.cart.get(i).devices.getPro_sale() / 100)) * Cart.cart.get(i).quantity));
        }
    ...
    public String add(@PathVariable("id") String id, @NonNull HttpServletRequest request) throws ClassNotFoundException {
        Devices devices = productService.show(Integer.parseInt(id));
        if (devices.getPro_amount() == 0) {
            return id;
        }
        Integer isUpdate = 0;
        for (int i = 0; i < Cart.cart.size(); i++) {
            if (Cart.cart.get(i).devices.getId() == Integer.parseInt(id)) {
    ...
    public ResponseEntity<MessageEntity> update(@PathVariable("id") String id, @PathVariable("quantity") String quantity, @NonNull HttpServletRequest request) throws ClassNotFoundException {
        Devices devices = productService.show(Integer.parseInt(id));
        if (devices.getPro_amount() < Integer.parseInt(quantity)) {
        ...
        UpdateCartResponse response = UpdateCartResponse.builder()
                ...
                .totalItem(CommonConfigurations.formatPrice((devices.getPro_price() - devices.getPro_price() * (devices.getPro_sale() / 100)) * Integer.parseInt(quantity), 0, 0))
