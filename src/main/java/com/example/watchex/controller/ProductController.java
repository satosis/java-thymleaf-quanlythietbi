...
public class ProductController {
    ...
    public String index(@RequestParam Map<String, String> params, Model model) {
        ...
        model.addAttribute("models", "/devices?keyword=" + params.get("keyword") );
        return "product/list";
    }
    @GetMapping("/{slug}")
    public String detail(@PathVariable("slug") String slug, Model model) throws ClassNotFoundException {
        ...
        Devices entityDevices = productService.show(product.getId());
        List<RatingDto> ratings = ratingRepository.getRatingProduct(product.getId());
        List<Rating> listRating = ratingRepository.getListRatingProduct(entityDevices);
        List<Rating> myRating = ratingRepository.getMyRatingProduct(entityDevices, user);
        List<Devices> devicesSuggest = productService.getProductsByCategory(product.getCategory().getId(), 3);
        UserFavourite userFavourite = userFavouriteService.getByProductId(entityDevices);
        String[] tags= product.getKeywordseo().split(",");
        model.addAttribute("cartCount", Cart.cart.size());
        model.addAttribute("product", product);
        model.addAttribute("ratings", ratings);
        model.addAttribute("categories", categories);
        model.addAttribute("productSuggest", devicesSuggest);
    ...
    public String category(@PathVariable("slug") String slug, @RequestParam Map<String, String> params, Model model) {
        ...
        Page<Devices> products = productService.findBySlugCategory(slug, params);
        model.addAttribute("cartCount", Cart.cart.size());
        model.addAttribute("products", products);
        model.addAttribute("categories", categories);
        model.addAttribute("currentPage", params.get("page") != null ? Integer.parseInt(params.get("page")) : 1);
        ...
        model.addAttribute("models", "/devices/category/" + slug);
        return "product/list";
    }
